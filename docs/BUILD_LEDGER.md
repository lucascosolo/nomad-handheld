...
| E2 | Hardware drivers: display (mock + **headless**), battery, sensors; richer display vocabulary (`display_card`/`display_list`/`display_choice`) | `src/nomad/hardware/**`, `src/nomad/mcp/hardware.py`, `tests/test_hardware.py` | `pytest tests/{test_hardware,test_mcp_hardware}.py` | TODO |
| E3 | Input: logical action layer, deadzone, repeat vs edge-trigger, extensible action set (D13, D26) | `src/nomad/input/**`, `tests/test_input.py` | `pytest tests/test_input.py` | TODO |
